#RAG which retrieved some items which is very close to the user query and its intent with higher number of recommended products k. It's considered as relevant context to augment LLMs
# Still we have problems or issues that the vector databases cannot explictly retrieve the most relevant products based on the user query.
...
def create_vector_detabase(styles,embeddings_model):
  """
...
def main():
    ...
    img_path_ls=get_random_images(styles3000,"/home/abdo/Downloads/LLMs_apps_gihub/visual_recommender_system/fashion_product_small/images/",8)
    st.title("RAG VS AI Recommender 🤖💡")
    ...
    * Get top-rank products Are highly related to your product.
    ...
    if search:
            user_intent=get_rephrased_query(product_description,intent_system_msg,model="gpt-4o")
            st.write("User intent:",user_intent)
            #intialize vector databases
            vec_db=create_vector_detabase(stock_items,OpenAIEmbeddings())
